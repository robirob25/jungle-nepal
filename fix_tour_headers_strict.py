...
class TagValidator(HTMLParser):
    ...
    def handle_endtag(self, tag):
        ...
        if self.stack and self.stack[-1] == tag:
            ...
            self.errors.append(f"Mismatched tag </{tag}> (expected </{self.stack[-1] if self.stack else 'NONE'}>)")

# Validate perfect_tour_header
val = TagValidator()
val.feed(perfect_tour_header)
if val.stack or val.errors:
    print(f"Header validation error! Stack: {val.stack}, Errors: {val.errors}")
    exit(1)
else:
    print("Header HTML is 100% syntactically perfect and balanced!")

# Apply to all 14 tour pages
...
for fname in os.listdir(tours_dir):
    ...
    content = re.sub(r'<header[^>]*>.*?</header>', perfect_tour_header, content, flags=re.DOTALL)

    with open(fpath, 'w', encoding='utf-8') as f:
        f.write(content)

print("Applied perfect, balanced header across all 14 tour pages!")
