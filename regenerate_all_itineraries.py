...
def build_tour_html(meta, raw_tour, is_english=False):
    ...
    price = raw_tour.get('price_discount', 'Sur devis')
    original_price = raw_tour.get('price_original', '')
    ...
    saving_badge_html = f'<span class="px-2.5 py-1 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-xs font-black">Promo</span>' if original_price else ''
    ...
    # EXTRACT FULL DETAILED DAYS ITINERARY FROM raw_tour["days"]
...
for raw_tour in raw_data:
    ...
    if val_fr.stack or val_fr.errors:
        ...
        with open(os.path.join(tours_dir, f"{short_id}.html"), 'w', encoding='utf-8') as f:
            f.write(fr_html)
        print(f"✅ FR {short_id}.html -> {len(days)} detailed days included & 0 DOM errors")
    ...
    if val_en.stack or val_en.errors:
        ...
        with open(os.path.join(en_tours_dir, f"{short_id}.html"), 'w', encoding='utf-8') as f:
            f.write(en_html)
        print(f"✅ EN {short_id}.html -> {len(days)} detailed days included & 0 DOM errors")

print(f"\n🎉 Successfully restored {total_days_count} detailed days of itineraries across all 14 tours!")
