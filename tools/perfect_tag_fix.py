...
tour_tags_map = {
    "tiji-mustang": "culture mustang-himalaya grand-tour",
    "immersion-spirituelle": "culture mustang-himalaya grand-tour",
    "nepal-sauvage": "safari culture mustang-himalaya grand-tour",
    "rara-lake-bardia": "safari aventure culture mustang-himalaya grand-tour",
    "carnet-de-voyage": "culture mustang-himalaya grand-tour",
    "rafting-safari": "rafting aventure bivouac safari grand-tour",
...
def update_file(filepath, is_en=False):
    ...
    if not is_en:
      ...
      <div id="categories-scroll-container" class="flex-1 flex items-center gap-2.5 overflow-x-auto no-scrollbar py-1 scroll-smooth">
        ...
        <button type="button" onclick="filterTrips('culture')" class="category-pill flex items-center gap-2 px-5 py-2.5 rounded-full bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs sm:text-sm whitespace-nowrap border border-slate-200/90 shadow-sm hover:shadow hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200 cursor-pointer shrink-0" data-filter="culture">
          <span>🏔️</span>
        ...
        new_select = """          <select id="search-dest" class="w-full bg-transparent text-slate-900 font-extrabold text-sm sm:text-base focus:outline-none cursor-pointer appearance-none">
            ...
            <option value="culture">🏔️ Himalaya, Mustang & Spiritualité</option>
      ...
      <div id="categories-scroll-container" class="flex-1 flex items-center gap-2.5 overflow-x-auto no-scrollbar py-1 scroll-smooth">
        ...
        new_select = """          <select id="search-dest" class="w-full bg-transparent text-slate-900 font-extrabold text-sm sm:text-base focus:outline-none cursor-pointer appearance-none">
            ...
            <option value="culture">🏔️ Himalayas, Mustang & Spirituality</option>
            <option value="grand-tour">🇳🇵 Grand 360° Expeditions</option>
          </select>"""

    # Locate start and end of category container
    ...
    new_js = f"""function filterTrips(category) {{
      ...
      cards.forEach(card => {{
        ...
        if (catLower === 'all') {{
          match = true;
        }} else if (catLower === 'culture' || catLower === 'mustang-himalaya' || catLower === 'mustang') {{
          match = cardCats.includes('culture') || cardCats.includes('mustang-himalaya') || cardCats.includes('mustang');
