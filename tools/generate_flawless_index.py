...
<body class="bg-safari-50 text-slate-800 font-sans antialiased selection:bg-jungle-900 selection:text-amber-200">
  ...
  <!-- 2. NAVBAR (Zéro retour à la ligne) -->
  <header id="main-nav" class="fixed top-8 left-0 right-0 z-40 transition-all duration-300 py-2 px-3 sm:px-6">
    ...
    <!-- Mobile Drawer -->
    <div id="mobile-menu" class="hidden xl:hidden mt-2 max-w-7xl mx-auto">
  ...
  <section class="relative min-h-[95vh] flex items-center justify-center pt-48 pb-36 px-4 sm:px-6 lg:px-8 overflow-hidden bg-jungle-950">
    ...
    <div class="relative z-10 max-w-6xl mx-auto text-center flex flex-col items-center w-full">
      ...
      <!-- ELONGATED LUXURIOUS SEARCH BAR (Single line, zero wrapping) -->
  ...
  <!-- 4. CAROUSEL DE FILTRES -->
  <section id="categories" class="py-6 bg-white border-b border-slate-200/80 sticky top-[68px] z-30 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center gap-2.5 overflow-x-auto no-scrollbar py-1">
        
        <button onclick="filterTrips('all')" class="category-pill active flex items-center gap-2 px-5 py-2.5 rounded-full bg-jungle-950 text-amber-300 font-extrabold text-xs sm:text-sm whitespace-nowrap shadow-md border border-amber-400/40 hover:scale-105 active:scale-95 transition-all duration-200" data-filter="all">
    ...
    function filterTrips(category) {{
      ...
      pills.forEach(pill => {{
        if (pill.dataset.filter === category) {{
          pill.classList.add('bg-jungle-950', 'text-amber-300', 'border-amber-400/40', 'scale-105');
          pill.classList.remove('bg-white', 'text-slate-800');
        }} else {{
          pill.classList.remove('bg-jungle-950', 'text-amber-300', 'border-amber-400/40', 'scale-105');
...
print("Flawless index.html regenerated successfully!")
