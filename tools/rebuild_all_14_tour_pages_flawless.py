...
flawless_template = """<!DOCTYPE html>
...
<body class="bg-[#faf8f5] text-slate-800 font-sans antialiased selection:bg-[#0e8354] selection:text-white">
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-24 font-sans">
    ...
    <nav class="flex items-center gap-2 text-xs text-slate-500 font-semibold mb-3 overflow-x-auto whitespace-nowrap">
      ...
      <span>›</span>
      <span class="text-slate-900 font-bold truncate">{title}</span>
    </nav>
    ...
    <div class="sticky top-[69px] z-30 bg-white/95 backdrop-blur-md border-b border-slate-200 py-3 mb-8">
      <div class="flex items-center gap-6 overflow-x-auto text-xs sm:text-sm font-bold text-slate-600">
        ...
        <a href="#programme" class="hover:text-[#0e8354] transition-colors pb-1 border-b-2 border-transparent hover:border-[#0e8354]">Itinéraire Jour par Jour ({days_count}j)</a>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
      ...
      <div class="lg:col-span-8 space-y-12">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200/90 shadow-sm">
            ...
            <div class="space-y-4">
              ...
              <div class="pt-2 border-t border-slate-100 flex items-center justify-between text-sm font-bold">
                ...
                <span class="text-[#0e8354] bg-emerald-50 px-3 py-0.5 rounded-full border border-emerald-200">{difficulty}</span>
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="programme" class="pt-6 border-t border-slate-200">
          <div class="flex items-center justify-between mb-6">
            ...
            <button onclick="toggleAllDays()" class="text-xs font-bold text-[#0e8354] hover:text-[#0c6d46] transition-colors">
      ...
      <div class="lg:col-span-4">
        <div id="booking-widget" class="sticky top-28 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/90 shadow-[0_12px_40px_rgba(0,0,0,0.08)] space-y-6">
          ...
          <div>
            <span class="text-[11px] font-extrabold uppercase tracking-widest text-[#0e8354]">{style}</span>
  ...
  <div id="lightbox-modal" class="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 hidden opacity-0 transition-opacity duration-300 flex items-center justify-center p-4">
    ...
    <div class="absolute top-6 left-6 right-6 flex items-center justify-between text-white z-10">
      ...
      <button onclick="closeLightbox()" class="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 flex items-center justify-center text-white transition-all hover:scale-105 active:scale-95" aria-label="Fermer">
        <i data-lucide="x" class="w-5 h-5"></i>
      </button>
    </div>

    <!-- Navigation Arrows -->
    <button onclick="prevLightboxImage(event)" class="absolute left-4 sm:left-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 hover:bg-black/90 border border-white/20 text-white flex items-center justify-center transition-all hover:scale-110 active:scale-95 z-20 shadow-2xl" aria-label="Photo précédente">
      <i data-lucide="chevron-left" class="w-6 h-6"></i>
    </button>
    <button onclick="nextLightboxImage(event)" class="absolute right-4 sm:right-8 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/60 hover:bg-black/90 border border-white/20 text-white flex items-center justify-center transition-all hover:scale-110 active:scale-95 z-20 shadow-2xl" aria-label="Photo suivante">
...
for raw_tour in raw_data:
    ...
    price = raw_tour.get('price_clean', 'Sur devis')
    original_price = raw_tour.get('original_price', '')
    saving_badge = raw_tour.get('saving_badge', '')

    original_price_html = f'<span class="text-xs sm:text-sm text-slate-400 line-through font-bold">{original_price}</span>' if original_price else ''
    saving_badge_html = f'<span class="px-2.5 py-1 rounded-full bg-rose-50 border border-rose-200 text-rose-700 text-xs font-black">{saving_badge}</span>' if saving_badge else ''
    ...
    # Itinerary days
    days = raw_tour.get('itinerary_days', [])
    days_html = ""
    for idx, d in enumerate(days):
        d_num = d.get('day_number', idx + 1)
        d_title = d.get('day_title', f'Jour {d_num}')
        d_desc = d.get('day_desc', '')
    ...
    radar = meta['radar']
    rendered_html = flawless_template.format(
        title=title_clean,
        title_encoded=title_encoded,
        overview=overview,
        overview_clean=overview_clean,
        hero_img=curated[0],
        gallery_img_1=curated[1],
        gallery_img_2=curated[2],
        gallery_img_3=curated[3],
        duration=duration,
        days_count=days_count,
        price=price,
        original_price_html=original_price_html,
        saving_badge_html=saving_badge_html,
        rating=meta['rating'],
    ...
    out_file = os.path.join(tours_dir, f"{short_id}.html")
    with open(out_file, 'w', encoding='utf-8') as f:
        f.write(rendered_html)

print("Regenerated all 14 official tour pages with 100% flawless layout, title, breadcrumbs and margins!")
