...
print(f"Total curated photos: {len(items)}")

cards_html = []
for idx, item in enumerate(items):
    # Hide items starting from index 20 (so first 20 items: idx 0 to 19 are visible initially in 'Tous')
    hidden_class = "hidden" if idx >= 20 else ""
...
new_gallery_section = f"""  <!-- ========================================================================= -->
  <!-- 5. GALERIE COLLAGE SAUVAGE DU NÉPAL (20 VISIBLES DANS TOUS + BOUTON VOIR +) -->
  <!-- ========================================================================= -->
...
{cards_joined}
      ...
      <!-- Voir + Button (In 'Tous' mode) -->
      <div class="mt-12 text-center" id="wildlife-see-more-container">
        <button onclick="seeMoreWildlife()" id="wildlife-see-more-btn" class="px-8 py-4 rounded-full bg-emerald-600 hover:bg-emerald-500 text-white font-black text-sm shadow-xl shadow-emerald-900/40 border border-emerald-400/40 hover:scale-105 active:scale-95 transition-all duration-300 cursor-pointer inline-flex items-center gap-2.5">
          <span>Voir + ({counts['all'] - 20} autres clichés)</span>
          <svg class="w-4 h-4 text-white animate-bounce" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M7 13l5 5 5-5M7 6l5 5 5-5"></path></svg>
        </button>
      </div>
...
wildlife_script_js = """  // WILDLIFE GALLERY CONTROLLER (20 INITIAL IN ALL + VOIR +)
  window.wildlifeData = """ + json.dumps(items, ensure_ascii=False) + """;
  window.currentWildlifeIndex = 0;
  window.isAllExpanded = false;
  ...
  window.filterWildlife = function(category) {
    ...
    var seeMoreContainer = document.getElementById('wildlife-see-more-container');
    ...
    if (category === 'all') {
      cards.forEach(function(card, idx) {
        if (!window.isAllExpanded && idx >= 20) {
      ...
      if (seeMoreContainer) {
        seeMoreContainer.style.display = window.isAllExpanded ? 'none' : 'block';
      }
    } else {
      // In specific category, show all 20 photos of that category
  ...
  window.seeMoreWildlife = function() {
    window.isAllExpanded = true;
    var cards = document.querySelectorAll('.wildlife-card');
    var seeMoreContainer = document.getElementById('wildlife-see-more-container');
    cards.forEach(function(card) {
      card.classList.remove('hidden');
...
print("Updated index.astro: 20 photos in 'Tous' with 'Voir +' button!")
