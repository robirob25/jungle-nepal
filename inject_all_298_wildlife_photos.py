...
# Sort items to showcase top diversity in the initial view (Tigers, Elephants, Rhinos, Birds, Crocs, etc.)
# Mix categories nicely
felins = [i for i in all_items if i['category'] == 'felins']
oiseaux = [i for i in all_items if i['category'] == 'oiseaux']
mammiferes = [i for i in all_items if i['category'] == 'mammiferes']
reptiles = [i for i in all_items if i['category'] == 'reptiles']

balanced_items = []
max_len = max(len(felins), len(oiseaux), len(mammiferes), len(reptiles))
for i in range(max_len):
    if i < len(felins): balanced_items.append(felins[i])
    if i < len(oiseaux): balanced_items.append(oiseaux[i])
    if i < len(mammiferes): balanced_items.append(mammiferes[i])
    if i < len(reptiles): balanced_items.append(reptiles[i])

# Add any remaining
seen = set()
ordered_items = []
for item in balanced_items + all_items:
    if item['file'] not in seen:
        seen.add(item['file'])
        ordered_items.append(item)

print(f"Total curated and sorted photos: {len(ordered_items)}")
...
for idx, item in enumerate(ordered_items):
    # Hide items beyond index 24 initially to ensure instant rendering, but keep in DOM for smooth expand
    hidden_class = "hidden" if idx >= 24 else ""
    card = f"""        <!-- Photo {idx+1}: {item['title']} -->
        <div class="wildlife-card {hidden_class} break-inside-avoid mb-6 relative rounded-3xl overflow-hidden group cursor-pointer border border-white/10 shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:border-emerald-500/50 hover:shadow-[0_20px_50px_rgba(14,131,84,0.25)] transition-all duration-300 bg-slate-900" data-category="{item['category']}" data-idx="{idx}" onclick="openWildlifeLightbox({idx})">
...
counts = {
    "all": len(ordered_items),
    "felins": sum(1 for i in ordered_items if i['category'] == 'felins'),
    "mammiferes": sum(1 for i in ordered_items if i['category'] == 'mammiferes'),
    "oiseaux": sum(1 for i in ordered_items if i['category'] == 'oiseaux'),
    "reptiles": sum(1 for i in ordered_items if i['category'] == 'reptiles'),
}

new_gallery_section = f"""  <!-- ========================================================================= -->
  <!-- 5. GALERIE COLLAGE SAUVAGE DU NÉPAL (298 PHOTOS DU DRIVE EN MASONRY FLUIDE) -->
  <!-- ========================================================================= -->
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <div class="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
        <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-md border border-emerald-400/30 text-amber-300 text-xs font-black uppercase tracking-widest mb-4 shadow-lg">
          <span>🌿 Galerie Complète des Expéditions Terrain</span>
        </div>
        ...
        <p class="mt-4 text-base sm:text-lg text-slate-300 font-medium leading-relaxed">
          Explorez l'intégralité des <strong>{counts['all']} photographies d'animaux sauvages</strong> prises lors de nos safaris à pied par nos voyageurs (Julien, Adrien, Frederic, Lucas, Antoine) et nos maîtres pisteurs.
        </p>
      </div>
      ...
      <!-- Fluid Multi-Column Masonry Collage (298 photos) -->
      <div class="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6" id="wildlife-grid">
{cards_joined}
      </div>

      <!-- Load More / Expand Full Collection Button -->
      <div class="mt-10 text-center" id="wildlife-load-more-container">
        <button onclick="showAllWildlifePhotos()" id="wildlife-load-more-btn" class="px-8 py-4 rounded-full bg-white/10 hover:bg-white/20 text-white font-extrabold text-xs sm:text-sm border border-white/20 backdrop-blur-md shadow-xl transition-all hover:scale-105 active:scale-95 cursor-pointer inline-flex items-center gap-2">
          <span>📸 Afficher l'intégralité des {counts['all']} photographies</span>
          <span>↓</span>
        </button>
      </div>
...
wildlife_script_js = """  // WILDLIFE GALLERY CONTROLLER (ALL 298 DRIVE PHOTOS)
  window.wildlifeData = """ + json.dumps(ordered_items, ensure_ascii=False) + """;
  ...
  window.filterWildlife = function(category) {
    ...
    var loadMoreBtn = document.getElementById('wildlife-load-more-container');
    ...
    if (category === 'all') {
      // In 'all' mode, show first 24 or all if expanded
  ...
  window.showAllWildlifePhotos = function() {
    var cards = document.querySelectorAll('.wildlife-card');
    var loadMoreBtn = document.getElementById('wildlife-load-more-container');
    cards.forEach(function(card) {
      card.classList.remove('hidden');
...
print(f"Successfully integrated all {len(ordered_items)} Drive wildlife photos into index.astro!")
