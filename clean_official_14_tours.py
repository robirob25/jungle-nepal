...
official_14_tours = [
    'bardia-explorateur.html',
    'chitwan-culture.html',
    'rafting-safari.html',
    'bardia-nuit-sauvage.html',
    'rara-lake-bardia.html',
    'bardia-babai-camping.html',
    'nepal-immersion-totale.html',
    'babai-special.html',
    'chitwan-bardia-complete.html',
    'tiji-mustang.html',
    'carnet-de-voyage.html',
    'jungle-extreme.html',
    'nepal-sauvage.html',
    'immersion-spirituelle.html'
]

tours_dir = '/Users/robinrozier/.gemini/antigravity/scratch/jungle-nepal/tours'

# Remove any non-official long slug files
for fname in os.listdir(tours_dir):
    if fname.endswith('.html') and fname not in official_14_tours:
        fpath = os.path.join(tours_dir, fname)
        os.remove(fpath)
        print(f"Removed legacy redundant file: {fname}")
...
for fname in official_14_tours:
    fpath = os.path.join(tours_dir, fname)
    ...
    # 3. Ensure share clean block is in place
    share_fav_regex = r'<div class=\"flex items-center gap-2\">\s*<button[^>]*>.*?Partager.*?</button>.*?</div>'
    c = re.sub(share_fav_regex, share_clean_block, c, flags=re.DOTALL)

    with open(fpath, 'w', encoding='utf-8') as f:
        f.write(c)

print(f"Verified and cleaned all {len(official_14_tours)} official tour pages!")
...
for fname in official_14_tours:
    ...
    hpos1 = c.find('<header')
    hpos2 = c.find('</header>')
    h_text = c[hpos1:hpos2+9]
    div_count = h_text.count('<div') - h_text.count('</div')
    nav_count = h_text.count('<nav') - h_text.count('</nav')
    has_fav = 'Favoris' in c
    print(f"Tour {fname:25}: Header Balance (div:{div_count}, nav:{nav_count}) | Favoris: {has_fav}")
